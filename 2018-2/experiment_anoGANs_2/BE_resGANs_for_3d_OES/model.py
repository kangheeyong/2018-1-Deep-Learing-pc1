...
import os
import time
from BE_resGANs_for_3d_OES.network import G, E, D_enc, D_dec
from BE_resGANs_for_3d_OES.utility import idx_shuffle, mnist_4by4_save, gan_loss_graph_save, my_roc_curve, my_hist, data_2d_to_3d
...
class BE_resGANs_for_3d_OES : 
    
    def __init__(self,sess, path, data, GANs_epoch = 100,E_epoch = 30, batch_size = 100, z_size = 100, lam = 0.01, 
                 gamma = 0.7, k_curr = 0.0, G_lr = 2e-4, G_beta1 = 0.5, E_lr = 2e-4, E_beta1 = 0.1, D_lr = 2e-5, 
                 D_beta1 = 0.5, feature_size = 100, minibatch_increase = False, factor_decrease = False) :
        ...
        self.z = tf.placeholder(tf.float32,shape=(None,1,1,1,self.z_size),name = 'z')    
        self.u = tf.placeholder(tf.float32, shape = (None,128, 128,128,1),name='u')     
    ...
    def GANs_fit(self) :
        ...
        for epoch in range(self.GANs_epoch) :
            ...
            for iteration in range(self.data.train_normal_data.shape[0] // temp_batch) : 
                ...
                train_images = data_2d_to_3d(self.data.train_normal_data[iteration*temp_batch : (iteration+1)*temp_batch]   )   
                u_ = np.reshape(train_images,(-1,128,128,128,1)) 
                z_ = np.random.uniform(-1,1,size=(temp_batch,1,1,1,self.z_size))                                                                                               
    ...
    def E_fit(self) : 
        ...
        for epoch in range(self.E_epoch) :
            ...
            for iteration in range(self.data.train_normal_data.shape[0] // self.batch_size) : 
                ...
                train_images = data_2d_to_3d(self.data.train_normal_data[iteration*self.batch_size : (iteration+1)*self.batch_size] )     
                u_ = np.reshape(train_images,(-1,128,128,128,1)) 
                z_ = np.random.uniform(-1,1,size=(self.batch_size,1,1,1,self.z_size))                                                           
    ...
    def report(self) : 
        ...
        for iteration in range(self.data.test_normal_data.shape[0] // self.batch_size) : 
            test_images = data_2d_to_3d(self.data.test_normal_data[iteration*self.batch_size : (iteration+1)*self.batch_size]   )   
            u_ = np.reshape(test_images,(-1,128,128,128,1)) 
            ...
            for i in range(self.batch_size) :
                ...
                residual_e = np.mean(np.sqrt((np.reshape(u_[i],(1,128*128*128))-np.reshape(im_re[i],(1,128*128*128)))**2))
                residual_cos = 1 - cosine_similarity(np.reshape(u_[i],(1,128*128*128)),np.reshape(im_re[i],(1,128*128*128)))
        ...
        for iteration in range(self.data.test_anomalous_data.shape[0] // self.batch_size) : 
            test_images = data_2d_to_3d(self.data.test_anomalous_data[iteration*self.batch_size : (iteration+1)*self.batch_size] )     
            u_ = np.reshape(test_images,(-1,128,128,128,1)) 
        ...
        for iteration in range(self.data.train_normal_data.shape[0] // self.batch_size) : 
            test_images = data_2d_to_3d(self.data.train_normal_data[iteration*self.batch_size : (iteration+1)*self.batch_size]  )    
            u_ = np.reshape(test_images,(-1,128,128,128,1)) 
