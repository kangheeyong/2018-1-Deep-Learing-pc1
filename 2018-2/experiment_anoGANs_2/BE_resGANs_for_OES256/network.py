...
def G(x,isTrain = True, reuse = False, name = 'G', z_size = 100) : #input = (minibatch * w * h * ch)
    ...
    with tf.variable_scope('G',reuse=reuse)  :
        ...
        conv0 = tf.layers.conv2d_transpose(x,64*n,[4,4], strides=(1,1),padding = 'valid',  kernel_initializer=w_init, bias_initializer=b_init)
        r0 = tf.nn.elu(tf.layers.batch_normalization(conv0,training=isTrain))   
               
        r1 = up_block(r0, 32*n, 'block_1', isTrain) #8*8*256(8*n)
        r2 = up_block(r1, 16*n, 'block_2', isTrain) #16*16*128(4*n)
        r3 = up_block(r2, 8*n, 'block_3', isTrain) #32*32*64(2*n)
        r4 = up_block(r3, 4*n, 'block_4', isTrain) #64*64*32(n)
        r5 = up_block(r4, 2*n, 'block_5', isTrain) #64*64*32(n)
        r6 = up_block(r5, n, 'block_6', isTrain) #64*64*32(n)
        conv7 = tf.layers.conv2d(r6,1,[3,3], strides=(1,1),padding = 'same', kernel_initializer=w_init, bias_initializer=b_init) #64*64*1

    return  tf.nn.sigmoid(conv7,name=name)
...
def E(x, isTrain = True, reuse = False, name = 'E', z_size = 100) : #input = (minibatch * w * h * ch)
    ...
    with tf.variable_scope('E',reuse=reuse)  :
        ...
        r6 = down_block(r5, 64*n, 'block_6', isTrain) #4*4*512(16*n)
        conv7 = tf.layers.conv2d(r6,z_size,[4,4], strides=(1,1),padding = 'valid', kernel_initializer=w_init, bias_initializer=b_init) #1*1*100

    return  tf.nn.tanh(tf.layers.batch_normalization(conv7,training=isTrain), name = name)#4*4*512
...
def D_enc(x,isTrain=True,reuse = False, name = 'D_enc', feature_size = 100) :
    ...
    with tf.variable_scope('D_enc', reuse=reuse) :
        ...
        r5 = down_block(r4, 32*n, 'block_5', isTrain) #4*4*512(16*n)
        r6 = down_block(r5, 64*n, 'block_6', isTrain) #4*4*512(16*n)
        conv7 = tf.layers.conv2d(r6,feature_size,[4,4], strides=(1,1),padding = 'valid', kernel_initializer=w_init, bias_initializer=b_init) 
        r7 = tf.layers.batch_normalization(conv7,training=isTrain)
    
    return tf.add(r7,0.0,name=name)
...
def D_dec(x,isTrain=True,reuse = False, name = 'D_dec') :
    ...
    return  tf.nn.sigmoid(conv7, name=name)#64*64*1
