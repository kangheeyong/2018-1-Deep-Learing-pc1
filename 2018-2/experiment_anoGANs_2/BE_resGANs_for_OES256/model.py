...
import os
import time
from BE_resGANs_for_OES256.network import G, E, D_enc, D_dec
from BE_resGANs_for_OES256.utility import idx_shuffle, mnist_4by4_save, gan_loss_graph_save, my_roc_curve, my_hist, mnist_matlab_4by4_save
...
class BE_resGANs_for_OES : 
    ...
    def __init__(self,sess, path, data, GANs_epoch = 100,E_epoch = 30, batch_size = 100, z_size = 100, lam = 0.01, gamma = 0.7, 
        ...
        self.u = tf.placeholder(tf.float32, shape = (None, 256,256,1),name='u')     
        ...
        self.E_u= E(self.u, self.isTrain,name = 'E_u', z_size = self.z_size) 

        self.re_image = G(self.E_u, self.isTrain, reuse=True, name ='re_image')
        #self.re_z = E(self.G_z, self.isTrain, reuse=True, name ='re_z',z_size = self.z_size)
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)) :  
            ...
            #self.E_optim = tf.train.AdamOptimizer(E_lr,beta1=E_beta1).minimize(self.re_z_loss, var_list=E_vars, name='E_optim')
    ...
    def GANs_fit(self) :
        ...
        mnist_4by4_save(np.reshape(self.data.test_normal_data[0:16],(-1,256,256,1)),self.path + '/GANs_result/normal')    
        mnist_matlab_4by4_save(np.reshape(self.data.test_normal_data[0:16],(-1,256,256,1)),self.path + '/GANs_result/normal')    
        mnist_4by4_save(np.reshape(self.data.test_anomalous_data[0:16],(-1,256,256,1)),self.path + '/GANs_result/anomalous')    
        mnist_matlab_4by4_save(np.reshape(self.data.test_anomalous_data[0:16],(-1,256,256,1)),self.path + '/GANs_result/anomalous') 
        ...
        for epoch in range(self.GANs_epoch) :
            ...
            for iteration in range(self.data.train_normal_data.shape[0] // temp_batch) : 
                ...
                u_ = np.reshape(train_images,(-1,256,256,1)) 
                ...
                _ , D_e, D_real_e, D_fake_e = self.sess.run([self.D_optim, self.D_loss, self.D_real_loss, self.D_fake_loss],
                                                     {self.u : u_, self.z : z_, self.k : self.k_curr, self.isTrain : True, self.factor : factor})
            ...
            print('D_e : %.3f, D_real_e : %.3f, D_fake_e : %.3f, G_e : %.3f, new_measure : %.3f, k_curr : %.3f'
                  %(np.mean(D_error), np.mean(D_real_error),np.mean(D_fake_error), np.mean(G_error),
                    np.mean(new_measure),self.k_curr))
            log_txt.write('D_e : %.3f, D_real_e : %.3f, D_fake_e : %.3f, G_e : %.3f, new_measure : %.3f, k_curr : %.3f\n'
                  %(np.mean(D_error), np.mean(D_real_error),np.mean(D_fake_error), np.mean(G_error),
                    np.mean(new_measure),self.k_curr))

            r = self.sess.run([self.G_z],feed_dict={self.z : test_z, self.isTrain : False})       
            mnist_4by4_save(np.reshape(r,(-1,256,256,1)),self.path + '/GANs_result/G_{}'.format(str(epoch).zfill(3)))
            mnist_matlab_4by4_save(np.reshape(r,(-1,256,256,1)),self.path + '/GANs_result/G_{}'.format(str(epoch).zfill(3)))
            
            r = self.sess.run([self.D_real],feed_dict={self.u : self.data.test_normal_data[0:16], self.isTrain : False})        
            mnist_4by4_save(np.reshape(r,(-1,256,256,1)),self.path + '/GANs_result/D_{}'.format(str(epoch).zfill(3)))
            mnist_matlab_4by4_save(np.reshape(r,(-1,256,256,1)),self.path + '/GANs_result/D_{}'.format(str(epoch).zfill(3)))
    ...
    def E_fit(self) : 
        ...
        test_z = np.random.uniform(-1,1,size=(16,1,1,self.z_size))
        mnist_4by4_save(np.reshape(self.data.test_normal_data[0:16],(-1,256,256,1)),self.path + '/E_result/normal')   
        mnist_matlab_4by4_save(np.reshape(self.data.test_normal_data[0:16],(-1,256,256,1)),self.path + '/E_result/normal')
        mnist_4by4_save(np.reshape(self.data.test_anomalous_data[0:16],(-1,256,256,1)),self.path + '/E_result/anomalous')    
        mnist_matlab_4by4_save(np.reshape(self.data.test_anomalous_data[0:16],(-1,256,256,1)),self.path + '/E_result/anomalous')    
        ...
        for epoch in range(self.E_epoch) :
            ...
            for iteration in range(self.data.train_normal_data.shape[0] // self.batch_size) : 
                ...
                #_  = self.sess.run([self.E_optim], {self.u : u_, self.z : z_, self.isTrain : True})

                _ , E_e = self.sess.run([self.E_AE_optim, self.re_image_loss], {self.u : u_, self.z : z_, self.isTrain : True})
                E_error.append(E_e)
            ...
            mnist_4by4_save(np.reshape(r,(-1,256,256,1)),self.path + '/E_result/normal_{}'.format(str(epoch).zfill(3)))
            mnist_matlab_4by4_save(np.reshape(r,(-1,256,256,1)),self.path + '/E_result/normal_{}'.format(str(epoch).zfill(3)))
            
            r = self.sess.run([self.re_image],feed_dict={self.u : self.data.test_anomalous_data[0:16],self.isTrain : False})        
            mnist_4by4_save(np.reshape(r,(-1,256,256,1)),self.path + '/E_result//anomlous_{}'.format(str(epoch).zfill(3)))
            mnist_matlab_4by4_save(np.reshape(r,(-1,256,256,1)),self.path + '/E_result//anomlous_{}'.format(str(epoch).zfill(3)))
    ...
    def report(self) : 
        ...
        for iteration in range(self.data.test_normal_data.shape[0] // self.batch_size) : 
            ...
            u_ = np.reshape(test_images,(-1,256,256,1)) 

            im_enc= self.sess.run([self.D_enc],{self.u : u_,self.isTrain : False})
            im_re, im_z= self.sess.run([self.re_image, self.E_u],{self.u : u_, self.isTrain : False})
            im_re_enc= self.sess.run([self.D_enc],{self.u : im_re, self.z : im_z, self.isTrain : False})
            ...
            for i in range(self.batch_size) :
                ...
                residual_e = np.mean(np.sqrt((np.reshape(u_[i],(1,256*256))-np.reshape(im_re[i],(1,256*256)))**2))
                residual_cos = 1 - cosine_similarity(np.reshape(u_[i],(1,256*256)),np.reshape(im_re[i],(1,256*256)))
