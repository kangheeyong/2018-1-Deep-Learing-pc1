...
def E(x, isTrain = True, reuse = False, name = 'E', z_size = 100) : #input = (minibatch * w * h * ch)
    ...
    with tf.variable_scope('E',reuse=reuse)  :
        ...
        conv6 = tf.layers.conv2d(r5,z_size,[4,4], strides=(1,1),padding = 'valid', kernel_initializer=w_init, bias_initializer=b_init) #1*1*100

    return  tf.nn.tanh(tf.layers.batch_normalization(conv6,training=isTrain), name = name)#4*4*512
...
def D_enc(x,isTrain=True,reuse = False, name = 'D_enc', feature_size = 100) :
    ...
    with tf.variable_scope('D_enc', reuse=reuse) :
        ...
        conv6 = tf.layers.conv2d(r5,feature_size,[4,4], strides=(1,1),padding = 'valid', kernel_initializer=w_init, bias_initializer=b_init) #1*1*100
        r6 = tf.layers.batch_normalization(conv6,training=isTrain)
    
    return tf.add(r6,0.0,name=name)
