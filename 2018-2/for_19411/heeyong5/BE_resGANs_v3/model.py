...
import os
import time
from BE_resGANs_v3.network import G, E, D_enc, D_dec, Q_cat
from BE_resGANs_v3.utility import idx_shuffle, mnist_4by4_save, gan_loss_graph_save, my_roc_curve, my_hist
...
class BE_resGANs_v3 : 
    ...
    def __init__(self,sess, path, data, GANs_epoch = 100,E_epoch = 30, batch_size = 100, z_size = 100, lam = 0.01, gamma = 0.7, k_curr = 0.0,
        ...
        self.G_z = G(self.z, self.c, self.isTrain, name='G_z', c_size = self.c_size, z_size = self.z_size) 
        self.E_u,  self.E_u_c = E(self.u, self.isTrain,name = 'E_u', c_size = self.c_size, z_size = self.z_size) 

        self.re_image = G(self.E_u, self.E_u_c, self.isTrain, reuse=True, name ='re_image', c_size = self.c_size, z_size = self.z_size)
