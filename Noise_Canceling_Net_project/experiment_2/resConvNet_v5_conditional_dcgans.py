...
def simple_D(x,y,ref,isTrain=True,reuse = False) :
    
    with tf.variable_scope('D', reuse=reuse) :
        cat1 = tf.concat([x,y,ref],3)        
...
ramda = tf.placeholder(dtype=tf.float32,name='ramda') 
...
D_real = simple_D(t,u,ref,isTrain)
D_fake = simple_D(G_y,u,ref,isTrain,reuse=True)
...
G_loss = tf.add(ramda*content_loss,gan_loss,name='G_loss')
...
for i in range(500000) :
    ...
    _ , G_e, gan_e, content_e= sess.run([G_optim, G_loss, gan_loss, content_loss],
        feed_dict={u : np.reshape(train_input,(-1,28,28,1)), ref : np.reshape(train_ref,(-1,28,28,1)),
            t : np.reshape(train_origin,(-1,28,28,1)),isTrain : True, ramda : ramda_gain})
    G_error.append(G_e)
